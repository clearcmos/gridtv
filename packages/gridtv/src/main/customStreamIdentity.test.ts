...
describe('custom stream identity', () => {
  ...
  it('reconstructs legacy IDs from source, label, or link', () => {
    expect([
      ...legacyCustomIdMap([
        {
          source: 'Camera One',
          label: 'ignored',
          link: 'https://example.com/one.m3u8',
          kind: 'video',
        },
        {
          label: 'Lobby',
          link: 'https://example.com/two.m3u8',
          kind: 'video',
        },
        { link: 'https://example.com/three.m3u8', kind: 'video' },
        { link: '', kind: 'video' },
      ]).keys(),
    ]).toEqual(['cam', 'lob', 'exa'])
  })
  ...
  it('preserves known and empty assignments during legacy migration', () => {
    const customEntries = addStableCustomStreamIds([
      { link: 'https://www.twitch.tv/lacy', kind: 'video' },
    ])
    const doc = new Y.Doc()
    const viewsState = doc.getMap<Y.Map<string | undefined>>('views')
    ;['known-stream', undefined].forEach((streamId, idx) => {
      const cell = new Y.Map<string | undefined>()
      cell.set('streamId', streamId)
      viewsState.set(String(idx), cell)
    })
    ...
    migrateLegacyCustomAssignments({
      ...
      transact: (fn) => doc.transact(fn),
      customEntries,
      knownStreamIds: new Set(['known-stream', 'twitch-lacy']),
    })

    expect(
      [...viewsState.values()].map((cell) => cell.get('streamId')),
    ).toEqual(['known-stream', undefined])
  })
})
